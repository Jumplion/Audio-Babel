...
namespace {
    ...
    auto roundKey(uint64_t key, size_t halfBits, int round) -> uint64_t {
        uint64_t state =
            key ^ (0x9E3779B97F4A7C15ULL * (static_cast<uint64_t>(halfBits) + 1)) ^
...
auto reconstructIndexFromPosition(const LibraryPosition& pos) -> cpp_int {
    ...
    uint64_t linearOffset =
        static_cast<uint64_t>(pos.wall)  * ITEMS_PER_WALL  +
        static_cast<uint64_t>(pos.shelf) * ITEMS_PER_SHELF +
        static_cast<uint64_t>(pos.album) * ITEMS_PER_ALBUM +
        static_cast<uint64_t>(pos.track);
