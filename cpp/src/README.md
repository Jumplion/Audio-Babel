...
| `Index.cpp` | `Index.h` | The PCM-payload ↔ big-integer bijection. Treats the payload as a stream of 16-bit little-endian samples (the `bitDepth`/`sampleRate` arguments only affect the cosmetic WAV header). Applies the optional scramble when `AUDIOBABEL_SCRAMBLE` is set. |
